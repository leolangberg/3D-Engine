...
int Light_Object_world8(Object* object, Camera* camera, Light* lights, int max_lights, int shading_mode, unsigned char* rgblookup) {
    ...
    for(int curr_poly = 0; curr_poly < object->num_polys; curr_poly++)
    {
        ...
        if(shading_mode == FLAT_SHADING)
        {
            ...
            for(int curr_light = 0; curr_light < max_lights; curr_light++)
            {
                ...
                if(lights[curr_light].attr & LIGHTV1_ATTR_AMBIENT)
                {
                    ...
                    printf("c_ambient.b: %d b_base: %d b_sum: %d\n", lights[curr_light].c_ambient.b, b_base, b_sum);
                }
                else
            ...
            printf("rgb index: %d\n", rgb_index);
            shaded_color  = rgb_index; 
            printf("color before: %d  new shade: %d\n", object->polys[curr_poly].color, shaded_color);
            object->polys[curr_poly].shade[0] = (int)((shaded_color << 16) | object->polys[curr_poly].color);
            object->polys[curr_poly].shade[1] = (int)((shaded_color << 16) | object->polys[curr_poly].color);
            object->polys[curr_poly].shade[2] = (int)((shaded_color << 16) | object->polys[curr_poly].color);
            object->polys[curr_poly].shade[3] = (int)((shaded_color << 16) | object->polys[curr_poly].color);

        } // end if FLAT_SHADING
        else 
        if(shading_mode == GOURAUD_SHADING)
        {
            ...
            for(int num_verts = 0; num_verts < object->polys[curr_poly].num_points; num_verts++)
            {
                // Determine points based on current vertice being processed.
                if(num_verts == 0)
                {
                    //printf("num_verts == 0\n");
                    vertex_0 = object->polys[curr_poly].vertex_list[0];
                ...
                {
                    // polygon is quad
                    vertex_0 = object->polys[curr_poly].vertex_list[3];
                    vertex_1 = object->polys[curr_poly].vertex_list[0];
                    vertex_2 = object->polys[curr_poly].vertex_list[2];
                }
                else {
                    // should not happen.
                    printf("num_verts illegal number: %d\n", num_verts);
                };
                ...
                object->polys[curr_poly].shade[num_verts] = (int)((shaded_color << 16) | object->polys[curr_poly].color);
                printf("         color previous: %x\n", object->polys[curr_poly].color);
                printf("new shade for vertice %d: %x\n", num_verts, object->polys[curr_poly].shade[num_verts]);
        ...
        else
        {
            ...
            object->polys[curr_poly].shade[0] = (int)((object->polys[curr_poly].color << 16) | object->polys[curr_poly].color);
        }
    } // end for poly
